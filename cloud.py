...
from utils import send_otp, verify_otp_code, hash_password, check_password, send_welcome_email
import os
import json
...
class UserServiceSkeleton(cloudsecurity_pb2_grpc.UserServiceServicer):
    ...
    def signup(self, request, context) -> cloudsecurity_pb2.Response:
        """Register a new user - NO OTP SENT"""
    ...
    def _signup_firebase(self, request):
        """Signup using Firebase - Send welcome email only"""
        # Check username
        ...
        # Send welcome email (NO OTP for signup)
        try:
            send_welcome_email(request.email, request.login)
            print(f"[SUCCESS] Welcome email sent to {request.email}")
        except Exception as e:
            print(f"[WARNING] Failed to send welcome email: {e}")
            # Don't fail signup if email fails

        return cloudsecurity_pb2.Response(
            result="Success: Signup successful. You may now login."
        )
    
    def _signup_local(self, request):
        """Signup using local database - Send welcome email only"""
        try:
    ...
    def _login_firebase(self, request):
        ...
        if result['success']:
            ...
            if 'successfully' in otp_result or 'OTP stored' in otp_result:
                return cloudsecurity_pb2.Response(
                    result=f"OTP sent to {user_email}. Please verify."
                )
            else:
                return cloudsecurity_pb2.Response(result=f"Error: {otp_result}")
        else:
            return cloudsecurity_pb2.Response(result="Unauthorized: Invalid credentials")
    
    def _login_local(self, request):
        """Login using local database - Send OTP"""
        user_email = request.login
        ...
        try:
            ...
            if result:
                ...
                if self.db_type == 'firebase':
                    ...
                    user_doc = self.users_ref.document(user.uid).get()
                    if user_doc.exists:
                        user_data = user_doc.to_dict()
                        token = f"firebase_token_{user.uid}"
                ...
                return cloudsecurity_pb2.Response(
                    result=f"Success: Login complete! Token: {token}, Role: {role}"
                )
            else:
